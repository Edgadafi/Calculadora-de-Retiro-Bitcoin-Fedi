...
3. **Mercado Pago.** [`api/mp-webhook.js`](../api/mp-webhook.js) reconsulta el pago por id con nuestro access token y registra la compra. Esa reconsulta es la que impide que un aviso forjado invente un cobro. La firma HMAC se valida **antes** de esa consulta, así una petición sin firma válida no cuesta una llamada a la API de Mercado Pago; en producción se falla cerrado si falta `MERCADOPAGO_WEBHOOK_SECRET`, y hay además un límite holgado de 60 peticiones por minuto por IP.
...
4. `MERCADOPAGO_WEBHOOK_SECRET` desde Dashboard de Mercado Pago → Webhooks. **Obligatorio en producción:** sin él el webhook responde 503 y no registra cobros, porque quedaría sin autenticar y cada petición costaría una consulta a la API de Mercado Pago. En local y preview la validación se omite.
